...
Warnings are generated to help prevent unexpected behaviour. For example if you simply mispelled one of the templated variable names in the template, this would be replaced by the default value. And without the above warnings you would never know that it happened. If you intended to specify all templated variables but you get this warning, check that the template you specified is correct! As we mentioned above, using a `data.frame` as a template is also possible. Here is an example.
...
if(requireNamespace("slimr", quietly = TRUE)) {
  out <- slimr::slim_run(script_txt, simple_run = TRUE, capture_output = TRUE)
...
```
`slimr` also provides a suite of handy functions to get this data into a `data.frame` or `tibble`

```{r tests, eval=FALSE}
if(requireNamespace("slimr", quietly = TRUE)) {

  test <- stringr::str_match_all(paste(out$output, collapse = "\n"), stringr::regex("<slimr_out:start>(.*?)<slimr_out:end>", dotall = TRUE))[[1]][ , 2]
  test2 <- readr::read_csv(test, col_names = c("generation", "name", "expression", "data"), quote = "\'")
}
```
